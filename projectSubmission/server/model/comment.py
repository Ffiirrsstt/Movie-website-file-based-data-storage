...
data = pd.read_csv('D:\มหาลัย\cpe270\projectSubmission\server\model\datasetsReview\pos.csv', sep='\t', encoding='utf-8')
data = pd.concat([data, pd.read_csv('D:\มหาลัย\cpe270\projectSubmission\server\model\datasetsReview\\neq.csv', sep='\t', encoding='utf-8')])
# data = pd.concat([data, pd.read_csv('D:\มหาลัย\cpe270\projectSubmission\server\model\datasetsReview\\test.csv', sep='\t', encoding='utf-8')])
...
trainData, testData, trainSentiment, testSentiment = train_test_split(padded, data['sentiment'], test_size=0.3, random_state=42)
...
model.save('projectSubmission\\server\\model\\sentiment\\model.h5')
# บันทึกข้อมูลตัวแปลงข้อความเป็นตัวเลข ,'wb' คือโหมด binary write
# pickle.dump บันทึกข้อมูล tokenizer ลงในไฟล์
with open('projectSubmission\\server\\model\\Sentiment\\tokenizer.pkl', 'wb') as file:
